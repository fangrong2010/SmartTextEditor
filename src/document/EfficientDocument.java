...
//		}
		for(int i = 0; i < tokens.size(); i++){
			...
			if(isWord(token)){ // token is a word
				numWords++;
				numSyllables += countSyllables(token);
				if(i == tokens.size()-1) // not ending with a punctuation, increment numSentences
					numSentences++;
			}
			else // token is a punctuation
				numSentences++;
		}
	}
	...
	public static void main(String[] args)
	{
		...
		testCase(new EfficientDocument("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, nam dolores explicari forensibus ad."),
		         32, 15, 1);
//		EfficientDocument d = new EfficientDocument("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, 7.5.");
//		d.processText();
