...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index < s.length()+1; index++){
			for(char c = 'a'; c <= 'z'; c++){
				StringBuffer sb = new StringBuffer(s);
				sb.insert(index, c);
				// check if the newly created string can be added to currentList
				String newWord = sb.toString(); // avoid doing sb.toString() multiple times
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && num < numSuggestions){
			...
			List<String> neighborWords = distanceOne(currWord, true);
