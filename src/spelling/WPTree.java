...
public class WPTree implements WordPath {
	...
	public WPTree () {
		...
		Dictionary d = new spelling.DictionaryBST();
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		WPTreeNode start = new WPTreeNode(word1, root);
		...
		while(!queue.isEmpty()){
			...
			List<String> neighborWords = nw.distanceOne(currWord, wordsOnly);
			for(String neighborWord : neighborWords){
				if(!visited.contains(neighborWord)){
					//if(nw.dict.isWord(neighborWord)){ // if neighborWord is a valid word
						WPTreeNode node = curr.addChild(neighborWord);   // add neighborWord as the child of curr
						if(neighborWord.equals(word2)){ // reaches word2
